...
%DIF ADD revision0.3.tex     Fri Jul 10 10:04:51 2020
%% ---------------------------------
...
\section{Dataset}
\cite{Penna2015} validated GPS-derived estimates of OTL for geophysical interpretation while \cite{Bos2015} followed the previous study to analyse OTL displacements in western Europe. As previously stated, to quality control our analysis we decided to follow the validation approach and study region of \cite{Penna2015}, but extended it to include GLONASS and GPS+GLONASS solutions. Aside from the addition of GLONASS data, an important difference to the original studies is the shift in time period from 2007.0–2013.0 to 2010.0–2014.0. This shift provides sufficient GLONASS data following the upgrade of many receivers to track \DIFdelbegin \DIFdel{GNSS from 2009. }\DIFdelend \DIFaddbegin \DIFadd{GLONASS from 2009 that followed the restoration of GLONASS constellation finished (24 SVs) in March 2010. }\DIFaddend Despite this covering a shorter time span, the length of continuous \DIFdelbegin \DIFdel{observation }\DIFdelend \DIFaddbegin \DIFadd{observations }\DIFaddend within each site (minimum availability of 0.95 through dataset) exceeds the recommended \DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{$\sim$}\DIFaddend 1000 days of continuous observations (4 years with 0.7 availability) \citep{Penna2015}. \DIFdelbegin \DIFdel{Additionally, }\DIFdelend \DIFaddbegin \DIFadd{While the }\DIFaddend selected time period is fully covered by a complete set of reprocessed orbit and clock products\DIFaddbegin \DIFadd{, the period analysed is the worst in terms of }\DIFaddend .

The sites used in our study are shown in Figure 1, with a focus on south-west England where a large $M_2$ OTL signal is present. Of the 21 stations, 14 stations are in south-west England\DIFdelbegin \DIFdel{and the southern part of Wales with an additional seven stations in Europe, all }\DIFdelend \DIFaddbegin \DIFadd{: covering both sides of Bristol channel (ANLX, SWAS, CARI, CAMO, PADT, APPL, TAUT) and northern coast of English Channel up to Herstmonceux (PMTH, PRAE, EXMO, PBIL, POOL, CHIO, SANO, HERT) with one site (BRST) in the south. Two sites are in northern England (WEAR, LOFT), two in Scotland (LERI, BRAE) with one site in central Europe (ZIM2). All sites are }\DIFaddend equipped with GPS+GLONASS receivers. \DIFdelbegin \DIFdel{Our station set is somewhat different to that }\DIFdelend \DIFaddbegin \DIFadd{Note that sites CAMO, LERI and ZIM2 sites replace CAMB, LERW and ZIMM respectively, that were }\DIFaddend used by \cite{Penna2015} \DIFdelbegin \DIFdel{due to the lack of GLONASS-capable receivers, replacement with sites a few metres away, or the addition of new sites which were deployed just before 2010.0}\DIFdelend \DIFaddbegin \DIFadd{because the earlier sites were replaced with newer ones, including adding GLONASS tracking}\DIFaddend .
...
Earth body-tide (EBT) and pole tides were modelled according to IERS Conventions \citep{IERS2010}. The OTL displacement within \DIFaddbegin \DIFadd{each }\DIFaddend processing run was modelled with the FES2004 tidal atlas \citep{Lyard2006} and elastic Green’s functions based on the Gutenberg-Bullen Earth model \citep{Farrell1972} (FES2004\_GBe), with centre-of-mass correction applied depending on the adopted orbit products. \DIFdelbegin \DIFdel{The OTL values were generated in BLQ format for 11 principal constituents ($M_2$, $S_2$, $N_2$, $K_2$, $K_1$, $O_1$, $P_1$, $Q_1$, $Mf$, $Mm$ and $Ssa$) using }\DIFdelend \DIFaddbegin \DIFadd{All OTL values used in the pulication were generated using CARGA software at the }\DIFaddend free ocean tide loading provider (http://holt.oso.chalmers.se/loading) \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
\cite{Bos2015}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , and a Green’s function based on the STW105 Earth model additionally corrected for dissipation at the $M_2$ frequency which we call STW105d (FES2014b\_STW105d).

\DIFaddbegin \DIFadd{We utilize box-and-whisker plots to demonstrate the distribution of the estimates with the boxes defined by the interquartile range (IQR), with median as a horizontal line, and whiskers by additional }\pm \DIFadd{1.5\times \text{IQR}.
}

\DIFaddend \section{Process noise optimization}
Process noise settings within GipsyX need to be chosen to ensure optimal separation of site displacement, tropospheric zenith delays, noise etc. For example, tight coordinate process noise value, even the default value of 0.57 mm/sqrt(s), tend to clip OTL amplitudes, especially in \DIFdelbegin \DIFdel{costal }\DIFdelend \DIFaddbegin \DIFadd{coastal }\DIFaddend sites. \cite{Penna2015} developed a method of tuning process noise values for GPS PPP, which we expanded to accommodate the additional major diurnal/semidiurnal constituents considered here, as well as the use of GPS and GLONASS data.

To do this, we used the CAMO site, the successor of CAMB used by \cite{Penna2015} and tested a range of coordinate and Zenith Wet Delay (ZWD) process noise settings exactly as described by \cite{Penna2015}. These tests focus on a range of metrics, namely the standard deviation of the height time series (shown as “Ht std/3”, as divided by 3), the standard deviation of kinematic ZWD normalized by ZWD values from a static solution (“ZWDstatic”), root mean square of the carrier phase residuals (“RMSres”), \DIFdelbegin \DIFdel{magnitude of }\DIFdelend $M_2$ residual OTL \DIFaddbegin \DIFadd{magnitude}\DIFaddend , $\|Z_{res}\|$, \DIFdelbegin \DIFdel{computed by differencing observed OTL with FES2004\_GBe theoretical values (“$M_2$”) }\DIFdelend and $\|Z_{res}\|$ of a synthetic \DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{$\sim$}\DIFaddend 13.96 h signal and its controlled, known input (designated “synth err”). \DIFdelbegin \DIFdel{The “synth err” $\|Z_{res}\|$ was estimated from solutions with harmonic }\DIFdelend \DIFaddbegin \DIFadd{We only demonstrate results with no synthetic }\DIFaddend signal introduced into sites’ nominal location\DIFdelbegin \DIFdel{(2, 4 and 6 mm into east, north and up components respectively). The extracted amplitudes were then compared with the known signal to measure the level of propagation into other components. }\DIFdelend \DIFaddbegin \DIFadd{, reason discussed in the supplementary material (Fig. S6).
}\DIFaddend %figure 2
\begin{figure*}[t]
...
\DIFdelbegin \DIFdel{The only difference in our results to those of \mbox{%DIFAUXCMD
\cite{Penna2015} }\hspace{0pt}%DIFAUXCMD
...
Given the known accuracy of the ocean tide models in this region, and small effects of errors in solid Earth models, our assumption is that as $\|Z_{res}\|$ approaches zero the estimates increase in accuracy, as in \cite{Bos2015}. \DIFaddbegin \DIFadd{Based on previous studies \mbox{%DIFAUXCMD
\citep[e.g.,][]{Yuan2013} }\hspace{0pt}%DIFAUXCMD
we expected $\|Z_{res}\|$ median values (up) of $\sim$2 mm for $K_2$ and $K_1$, $\sim$1 mm for $M_2$, $S_2$, $P_1$ and $\sim$0.5 mm for $N_2$, $O_1$, $Q_1$. }\DIFaddend Figure 3 shows GPS, GLONASS and GPS+GLONASS $\|Z_{res}\|$ estimates for east, north and up coordinate components. The combined GPS+GLONASS solutions perform either at the same level as GPS AR ($M_2$, $O_1$, $Q_1$) or better ($N_2$, $P_1$) for the up component. $\|Z_{res}\|$ values are smaller and more consistent for the east ($M_2$, $N_2$, $O_1$) and north ($M_2$, $N_2$, $P_1$) components respectively. 
...
Our results suggest that no single solution provides consistently better constituent estimates across all coordinate components. We suggest that optimum results are obtained using GPS+GLONASS for $M_2$, $N_2$, $O_1$, $P_1$ and $Q_1$, and GLONASS for $K_2$ and $K_1$, noting that GPS AR performs better for \DIFdelbegin \DIFdel{the $K_2$ and $K_1$ }\DIFdelend \DIFaddbegin \DIFadd{all constituents }\DIFaddend in the east component.
...
GPS $\|Z_{res}\|$ estimates show \DIFdelbegin \DIFdel{inverse }\DIFdelend \DIFaddbegin \DIFadd{similar }\DIFaddend behaviour in terms of $\|Z_{res}\|$ bias between ESA and CODE solutions in the up component (blue, Figure 6) \DIFdelbegin \DIFdel{and an additional linear dependency of increasing }\DIFdelend \DIFaddbegin \DIFadd{but ESA solutions' }\DIFaddend median $\|Z_{res}\|$ \DIFdelbegin \DIFdel{as a function of increasing elevation cutoff in the north component}\DIFdelend \DIFaddbegin \DIFadd{values are $\sim$1 mm larger for all elevation cutoff angle solutions}\DIFaddend .
Both ESA and CODE GPS+GLONASS $S_2$ results (green, Figure 6) show a blend of the two patterns observed with GPS and GLONASS solutions. GPS+GLONASS $S_2$ shows less sensitivity to the cutoff angle change than \DIFdelbegin \DIFdel{the GLONASS }\DIFdelend \DIFaddbegin \DIFadd{GLONASS or GPS }\DIFaddend solutions with both products. \DIFdelbegin \DIFdel{However, the CODE GPS+GLONASS $S_2$ solution consistently produces smaller $\|Z_{res}\|$ biases which may be related to the absence of $\|Z_{res}\|$ bias in the GPS solution and its higher weight in the solution due to higher number of simultaneously visible SVs. }\DIFdelend The effect was also studied with JPL products and AR: GPS and GPS AR; see Sect. 5.7.
...
GLONASS $K_2$ and $K_1$ estimates in the north have \DIFdelbegin \DIFdel{the tightest distribution of the }\DIFdelend \DIFaddbegin \DIFadd{lowest variance in }\DIFaddend $\|Z_{res}\|$ \DIFdelbegin \DIFdel{, are most stable for the range of }\DIFdelend \DIFaddbegin \DIFadd{and most stable towards changes in }\DIFaddend elevation cutoff angles and products. For the east component, CODE products GLONASS struggles to compete with GPS+GLONASS in terms of \DIFdelbegin \DIFdel{minimised distribution of }\DIFdelend $\|Z_{res}\|$ \DIFaddbegin \DIFadd{distribution variance }\DIFaddend ($K_1$) and elevation cutoff stability ($K_2$ and $K_1$). The \DIFdelbegin \DIFdel{GLONASS }\DIFdelend \DIFaddbegin \DIFadd{ESA products GLONASS, however, performs better for }\DIFaddend $K_1$ east \DIFdelbegin \DIFdel{is not true for the ESA products solutions which overperform the }\DIFdelend \DIFaddbegin \DIFadd{than the }\DIFaddend respective GPS+GLONASS in terms of $\|Z_{res}\|$ distribution consistency and median (\DIFdelbegin \DIFdel{see supplementary Figure $S_2$)and this explains the minor difference between ESA and CODE GLONASS solutions (Figure 5, top left)}\DIFdelend \DIFaddbegin \DIFadd{Fig. S2)}\DIFaddend . Elevation cutoff stability of ESA $K_2$ and $K_1$ in the east component is exactly as with CODE – best with GPS+GLONASS.

The up component of $K_2$ and $K_1$ is the most problematic, showing high $\|Z_{res}\|$ values with all constellation modes. GLONASS OTL values from both ESA and CODE solutions have \DIFdelbegin \DIFdel{tightest distributions and }\DIFdelend smallest $\|Z_{res}\|$ \DIFaddbegin \DIFadd{distribution variances and }\DIFaddend median values overperforming JPL GPS AR. Note that GPS+GLONASS $K_2$ up has marginally smaller median $\|\Delta Z_{res}\|$ in the elevation cutoff test, possibly due to higher amount of SVs, however, $\|Z_{res}\|$ values recovered are too high with major differences between CODE and ESA products’ values.

While we cannot certainly select a single constellation configuration optimal for all components of $K_2$ and $K_1$, we can conclude that based on our analysis, GLONASS \DIFdelbegin \DIFdel{performs best }\DIFdelend \DIFaddbegin \DIFadd{returns smaller $\|Z_{res}\|$ }\DIFaddend in the $K_2$ and $K_1$ north and up components while east component might show better results with GPS+GLONASS ($K_1$, CODE) but, due to \DIFdelbegin \DIFdel{better consistency }\DIFdelend \DIFaddbegin \DIFadd{higher closeness }\DIFaddend between products, GLONASS \DIFdelbegin \DIFdel{values are }\DIFdelend \DIFaddbegin \DIFadd{constellation is }\DIFaddend still preferred. The only exception to the east component conclusions is GPS AR (see Sect. 5.7).
...
\caption{Comparison of residual constituents’ estimates from GPS (top) and GPS AR (bottom) JPL native solutions. Grey crosses are as per Figure 3. As seen, most of constituents’ $\|Z_{res}\|$ distributions \DIFdelbeginFL \DIFdelFL{became tighter }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{variances }\DIFaddendFL and medians \DIFaddbeginFL \DIFaddFL{got }\DIFaddendFL smaller while $S_2$ $\|Z_{res}\|$ bias got removed with enabling of AR.}
\end{figure*}
Importantly, Figure 9 shows that enabling AR eliminates $\|Z_{res}\|$ bias in GPS and aligns the residual vectors with ESA/CODE GPS+GLONASS (Figure 3). Thus, the $S_2$ $\|Z_{res}\|$ bias was once again assessed with various elevation cutoff angles solutions. JPL GPS solutions, in the up component (Figure 10, left), show $S_2$ $\|Z_{res}\|$ bias to be constant between cutoff angles at about 1 mm with median $\|Z_{res}\|$ fluctuating around 3 mm. Similar behaviour was previously observed with solutions using ESA products \DIFaddbegin \DIFadd{(Fig.6)}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdel{Changing }\DIFdelend \DIFaddbegin \DIFadd{While the goal of reprocessing campaign was to preserve consistency with operational products \mbox{%DIFAUXCMD
\citep{Griffiths2019}}\hspace{0pt}%DIFAUXCMD
, based on previous results, we assumed that changing }\DIFaddend satellite orbit and clock products may produce substantial differences in \DIFdelbegin \DIFdel{results}\DIFdelend \DIFaddbegin \DIFadd{problematic solar-related constituents ($S_2$, $K_2$, $K_1$, $P_1$)}\DIFaddend . Thus, we \DIFaddbegin \DIFadd{first }\DIFaddend performed a comparison of ESA repro2 solutions (2010.0-2014.0) with the ESA operational product (2014.0-2019.0) which \DIFdelbegin \DIFdel{showed no significant changes in terms of $\|\Delta Z_{res}\|$ for $M_2$, $N_2$, $O_1$, $P_1$ and $Q_1$ estimated (Figure 12}\DIFdelend \DIFaddbegin \DIFadd{confirmed the hypothesis (Figure 11}\DIFaddend ). GLONASS \DIFdelbegin \DIFdel{, however, shows significant difference between two datasets }\DIFdelend \DIFaddbegin \DIFadd{$\|\Delta Z_{res}\|$ are still of minimal variance }\DIFaddend for $K_1$ and $K_2$ \DIFaddbegin \DIFadd{compared with GPS and GPS+GLONASS but are significant}\DIFaddend , up specifically, which might be related to the changes in GLONASS products processing. Considering the $S_2$, \DIFdelbegin \DIFdel{$\|\Delta Z_{res}\|$ shows }\DIFdelend the very same form of bias as previously seen with 2010.0-2014.0 dataset which suggests symmetric deviation of repro2 and operational products solutions from modelled value. \DIFaddbegin \DIFadd{The same explanation can be applied to the GPS-only $P_1$ up 0.5 mm $\|\Delta Z_{res}\|$ bias.
}\DIFaddend 

\DIFaddbegin \DIFadd{The results shown in Figure 12 are a composition of reprocessed products and operational products (years 5 to 9). We focus on $S_2$ up, to the evolution of inverted $\|Z_{res}\|$ bias and $K_1$ up, as most problematic diurnal constituents. The results align with general conclusions of \mbox{%DIFAUXCMD
\cite{Yuan2013} }\hspace{0pt}%DIFAUXCMD
...
Considering the above, we suggest that estimation of $K_1$ and $K_2$ constituents is best undertaken using GLONASS only solutions with an emphasis towards north component where it is most stable. $M_2$, $S_2$, $N_2$, $O_1$ and $Q_1$ can be reliably estimated from combined GPS+GLONASS or GPS AR solutions while $P_1$ is best with GPS+GLONASS.
