...
%DIF DEL revision0.5.tex   Mon Jul 27 11:12:27 2020
%DIF ADD revision0.6.tex   Wed Aug  5 14:12:03 2020
%% ---------------------------------
...
Recent studies have made use of GPS-derived OTL to study dissipation or anelastic dispersion effects in the shallow asthenosphere at the $M_2$ frequency \citep[e.g.][]{Bos2015}. This type of investigation has not been easily done previously due to various limiting factors such as the accuracy of ocean tide models and the quality and availability of \DIFdelbegin \DIFdel{GNSS }\DIFdelend \DIFaddbegin \DIFadd{GPS }\DIFaddend observations. Recently, however, \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{models }\DIFaddend have improved dramatically with \DIFdelbegin \DIFdel{altimeter-constrained ocean tide models \mbox{%DIFAUXCMD
\citep{Stammer2014}}\hspace{0pt}%DIFAUXCMD
and expanding GNSS networks . These have allowed for the demonstration }\DIFdelend \DIFaddbegin \DIFadd{the use of satellite altimetry \mbox{%DIFAUXCMD
\citep{Stammer2014}}\hspace{0pt}%DIFAUXCMD
, and GNSS networks have both expanded and have improved data quality. Together, this has enabled the exploration }\DIFaddend of limitations in the global seismic Preliminary Reference Earth Model (PREM) \citep{Dziewonski1981} with GPS observations in the western United States \citep{Ito2011,Yuan2012}, western Europe \citep{Bos2015}, South America \citep{Martens2016}, Eastern China Sea region \citep{Wang2019} and globally \citep{Yuan2013}. These limitations are associated partially with the incompatibility of the elastic parameters within the seismic (1 s period) and the tidal frequency bands and the anelasticity of the upper layers of the Earth, particularly the asthenosphere. \DIFdelbegin \DIFdel{Considering the }\DIFdelend \DIFaddbegin \DIFadd{The latter was studied through modelling the GPS-observed residuals of the }\DIFaddend major lunar tidal constituent, \DIFdelbegin \DIFdel{M2, }\DIFdelend \DIFaddbegin \DIFadd{$M_2$, by }\DIFaddend \cite{Bos2015} and, later, \cite{Wang2019}\DIFdelbegin \DIFdel{concluded that GPS-derived OTL displacement estimates were different to modelled displacements due to asthenospheric anelasticity. \mbox{%DIFAUXCMD
\cite{Lau2017} }\hspace{0pt}%DIFAUXCMD
used results }\DIFdelend \DIFaddbegin \DIFadd{, while \mbox{%DIFAUXCMD
\cite{Lau2017} }\hspace{0pt}%DIFAUXCMD
...
Aside from the addition of GLONASS data, an important difference to the study of \cite{Penna2015} is the shift in time period from 2007.0–2013.0 to 2010.0–2014.0. This shift provides sufficient GLONASS data following the upgrade of many receivers to track GLONASS from 2009 that followed the restoration of \DIFaddbegin \DIFadd{the }\DIFaddend GLONASS constellation finished \DIFaddbegin \DIFadd{in March 2010 }\DIFaddend (24 SVs)\DIFdelbegin \DIFdel{in March 2010. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend Despite this covering a shorter time span, the length of continuous observations \DIFdelbegin \DIFdel{within }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend each site (minimum availability of \DIFdelbegin \DIFdel{0.95 }\DIFdelend \DIFaddbegin \DIFadd{95\% }\DIFaddend through the dataset) exceeds the recommended $\sim$1000 days of continuous observations (4 years with \DIFdelbegin \DIFdel{0.7 }\DIFdelend \DIFaddbegin \DIFadd{70\% }\DIFaddend availability) \citep{Penna2015}. \DIFdelbegin \DIFdel{While the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend selected time period is fully covered by a complete \DIFaddbegin \DIFadd{and homogeneous }\DIFaddend set of reprocessed orbit and clock products.
...
We adopted a PPP solution approach and estimated station positions every 5 minutes with a random walk model introducing estimated optimum between-epoch constraints on coordinate evolution. We used the VMF1 gridded troposphere mapping function, based on the European Centre for Medium-Range Weather Forecasts (ECMWF) numerical weather model \citep{Boehm2006}. Additionally, ECMWF values for the hydrostatic zenith delay and wet zenith delay were used as a priori values for stochastic estimation of the wet zenith delay as a random walk process with optimum process noise values (Sect. 4) and tropospheric gradients were estimated as a random walk process \citep{Bar-Sever1998}, with process noise at 0.005 mm/sqrt(s). An elevation cut-off angle of seven degrees was applied, sufficient to maximize the \DIFdelbegin \DIFdel{amount }\DIFdelend \DIFaddbegin \DIFadd{number }\DIFaddend of GLONASS observations at the \DIFdelbegin \DIFdel{sites' latitude according to }\DIFdelend \DIFaddbegin \DIFadd{respective site latitude as noted by }\DIFaddend \cite{abbaszadeh_benefits_2020}, together with observation weights that were a function of the square-root of the sine of the satellite elevation angle.

Earth body-tide (EBT) and pole tides were modelled according to IERS 2010 Conventions \citep{IERS2010}. The OTL displacement within each processing run was modelled with the FES2004 tidal atlas \citep{Lyard2006} and elastic Green’s functions based on the Gutenberg-Bullen Earth model \citep{Farrell1972} (referred to as FES2004\_GBe), with centre-of-mass correction applied depending on the adopted orbit products. \DIFdelbegin \DIFdel{All OTL values used in the publication were generated using the CARGA software }\DIFdelend \DIFaddbegin \DIFadd{The FES2004-based OTL values were computed }\DIFaddend at the free ocean tide loading provider \DIFaddbegin \DIFadd{that uses OLFG/OLMP software }\DIFaddend (http://holt.oso.chalmers.se/loading) \DIFaddbegin \DIFadd{while the rest of OTL values used in this publication were computed with CARGA software }\DIFaddend \citep{bos_baker_2005}. We did not model atmospheric \DIFdelbegin \DIFdel{S2 }\DIFdelend \DIFaddbegin \DIFadd{$S_2$ }\DIFaddend tidal displacements.
...
}\DIFaddend , and small effects of errors in solid Earth models, our assumption is that as $\|Z_{res}\|$ approaches zero as the estimates increase in accuracy, \DIFdelbegin \DIFdel{as in }\DIFdelend \DIFaddbegin \DIFadd{also shown by }\DIFaddend \cite{Bos2015}. Based on previous studies \citep[e.g.,][]{Yuan2013} we expected $\|Z_{res}\|$ median values (up \DIFaddbegin \DIFadd{component}\DIFaddend ) of $\sim$2 mm for $K_2$ and $K_1$, $\sim$1 mm for $M_2$, $S_2$, $P_1$ and $\sim$0.5 mm for $N_2$, $O_1$, $Q_1$. 
...
We now explore the sensitivity of our solutions to different products and analysis choices starting with elevation cutoff angle sensitivity, which particularly affects the amount of multipath influence on the coordinate time series. We pay particular attention to $S_2$, $K_2$, $P_1$ and $K_1$ given the large systematic errors evident in GPS only solutions. We follow with an inter-comparison of solutions using various products and then \DIFdelbegin \DIFdel{assessing }\DIFdelend \DIFaddbegin \DIFadd{assess }\DIFaddend the impact of integer ambiguity resolution (GPS only). Finally, we test the stability of the constituent estimates to time series length. 

\subsection{Satellite orbit and clock products sensitivity tests}
We assessed whether the solutions were sensitive to changes in satellite-elevation cutoff angle. Three additional cutoff angle scenarios were tested: 10°, 15° and 20° (in addition to the default 7° cutoff angle). Different elevation angle cutoffs will significantly alter the observation geometry as well as modulate the expression of signal multipath into solutions, decreasing \DIFdelbegin \DIFdel{its amount }\DIFdelend \DIFaddbegin \DIFadd{the likely influence of multipath }\DIFaddend with higher cutoff values.

Figure 4 (top) shows the magnitude of vector difference, $\|\Delta Z_{res}\|$, between $Z_{res}$ values estimated from the 7° and 20° solutions and CODE products in both cases (upper subplot). $S_2$, $K_2$, $K_1$ and $P_1$ constituents in the up coordinate component show \DIFdelbegin \DIFdel{large mean magnitude }\DIFdelend \DIFaddbegin \DIFadd{larger mean magnitudes }\DIFaddend of vector differences in both GPS (0.56, 2.29, 2.88, 0.54 mm, respectively) and GLONASS (0.82, 0.64, 1.01, 0.58 mm, respectively) with the rest of constituents showing differences of less than 0.5 mm. GPS+GLONASS shows \DIFaddbegin \DIFadd{the }\DIFaddend smallest $\|\Delta Z_{res}\|$ between 7° and 20° \DIFaddbegin \DIFadd{cutoff }\DIFaddend estimates for $S_2$ and $P_1$ (0.31, 0.23 mm, respectively) and an additional decrease in $\|\Delta Z_{res}\|$ for $M_2$, $S_2$,$N_2$, $O_1$, $Q_1$ in the up component\DIFdelbegin \DIFdel{, which indicate higher }\DIFdelend \DIFaddbegin \DIFadd{. The results show high }\DIFaddend closeness of compared OTL values \DIFdelbegin \DIFdel{– higher stability towards cutoff angle change}\DIFdelend \DIFaddbegin \DIFadd{which indicates the high stability of GPS+GLONASS estimates towards changing cutoff angles}\DIFaddend .
...
GLONASS $K_2$ and $K_1$ estimates in the north have the lowest variance in $\|Z_{res}\|$ and are most stable with different elevation cutoff angles and products. For the east component, CODE products with GLONASS have larger $\|Z_{res}\|$ median and scatter than with GPS+GLONASS for $K_1$ and in terms of elevation cutoff stability ($K_2$ and $K_1$). Solutions using the ESA GLONASS products, however, \DIFdelbegin \DIFdel{performs }\DIFdelend \DIFaddbegin \DIFadd{perform }\DIFaddend better for $K_1$ east than the respective GPS+GLONASS in terms of $\|Z_{res}\|$ distribution consistency and median (Fig. S2). Elevation cutoff stability of ESA $K_2$ and $K_1$ in the east component is best with GPS+GLONASS as also found \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{when using }\DIFaddend CODE products.

The up component of $K_2$ and $K_1$ is the most problematic, showing high $\|Z_{res}\|$ values with all constellation modes. GLONASS OTL values using either both ESA or CODE products have the smallest \DIFaddbegin \DIFadd{medians and variances of }\DIFaddend $\|Z_{res}\|$\DIFdelbegin \DIFdel{distribution variances and median values and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend outperforming JPL GPS AR. Note that GPS+GLONASS $K_2$ up has a marginally smaller median $\|\Delta Z_{res}\|$ in the elevation cutoff test \DIFdelbegin \DIFdel{then that of GLONASS-only}\DIFdelend \DIFaddbegin \DIFadd{than that of GLONASS only}\DIFaddend , possibly due to the larger number of total satellites, however, both $K_2$ and $K_1$ $\|Z_{res}\|$ \DIFdelbegin \DIFdel{posses }%DIFDELCMD < \sim1%%%
\DIFdel{.5 }\DIFdelend \DIFaddbegin \DIFadd{suggest a $\sim$1.5 }\DIFaddend mm bias.

While we cannot \DIFdelbegin \DIFdel{certainly }\DIFdelend \DIFaddbegin \DIFadd{definitively }\DIFaddend select a single constellation configuration optimal for all components of $K_2$ and $K_1$, we can conclude that based on our analysis, GLONASS solutions have smaller $\|Z_{res}\|$ in the $K_2$ and $K_1$ north and up components while the east component shows better results with GPS+GLONASS ($K_1$, CODE). However, we recommend GLONASS-only solutions due to the higher level of agreement between solutions using ESA and CODE products. The only exception is the east component where the preference is for JPL GPS AR (see Sect. 5.7).
...
We find that the optimal constellation mode varies across all eight major tidal constituents \DIFdelbegin \DIFdel{. Alternatively, this optimal solution can be constructed from the combination of constellation modes for each constituent and component for the case of GPS and GLONASS presence. }%DIFDELCMD < 
