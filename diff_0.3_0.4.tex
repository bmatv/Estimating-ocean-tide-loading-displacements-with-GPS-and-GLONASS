...
%DIF DEL revision0.3.tex   Fri Jul 10 10:04:51 2020
%DIF ADD revision0.4.tex   Wed Jul 22 20:41:26 2020
%% ---------------------------------
...
Recent studies have made use of GPS-derived OTL to study dissipation or anelastic dispersion effects in the shallow asthenosphere at the $M_2$ frequency \citep[e.g.][]{Bos2015}. This type of investigation has not been easily done previously due to various limiting factors such as the accuracy of ocean tide models and the quality and availability of GNSS observations. \DIFdelbegin \DIFdel{Accuracy advances achieved with recent }\DIFdelend \DIFaddbegin \DIFadd{Recently, however, these have improved dramatically with }\DIFaddend altimeter-constrained \DIFdelbegin \DIFdel{ocean-tide }\DIFdelend \DIFaddbegin \DIFadd{ocean tide }\DIFaddend models \citep{Stammer2014} and \DIFdelbegin \DIFdel{evolving GNSS networkshas enabled studies to identify }\DIFdelend \DIFaddbegin \DIFadd{expanding GNSS networks. These have allowed for the demonstration of }\DIFaddend limitations in the global seismic Preliminary Reference Earth Model (PREM) \citep{Dziewonski1981} with GPS observations in the western United States \citep{Ito2011,Yuan2012}, western Europe \citep{Bos2015}, South America \citep{Martens2016}, Eastern China Sea region \citep{Wang2019} and globally \citep{Yuan2013}. These limitations are associated partially with the incompatibility of the elastic parameters within the seismic (1 s period) and the tidal frequency bands and the anelasticity of the upper layers of the Earth, particularly the asthenosphere. \DIFaddbegin \DIFadd{Considering the major lunar tidal constituent, M2, }\DIFaddend \cite{Bos2015} and, later, \cite{Wang2019} concluded that GPS-derived OTL displacement estimates were different to modeled displacements due to asthenospheric anelasticity. \cite{Lau2017} used results from the global study of \cite{Yuan2013} to constrain Earth’s deep-mantle buoyancy.
...
\cite{Penna2015}}\hspace{0pt}%DIFAUXCMD
, but extended it to include GLONASS and GPS+GLONASS solutions. Aside from the addition of GLONASS data, an important difference to the original studies is the shift in time period from 2007.0–2013.0 to 2010.0–2014.0. This shift provides sufficient GLONASS data following the upgrade of many receivers to track GLONASS from 2009 that followed the restoration of GLONASS constellation finished (24 SVs) in March 2010. Despite this covering a shorter time span, the length of continuous observations within each site (minimum availability of 0.95 through dataset) exceeds the recommended $\sim$1000 days of continuous observations (4 years with 0.7 availability) \mbox{%DIFAUXCMD
\citep{Penna2015}}\hspace{0pt}%DIFAUXCMD
. While the selected time period is fully covered by a complete set of reprocessed orbit and clock products, the period analysed is the worst in terms of .
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The sites used in our study are shown in Figure 1, with a focus on south-west England where a large $M_2$ OTL signal is present. Of the 21 stations, 14 stations are in south-west England: covering both sides of Bristol channel (ANLX, SWAS, CARI, CAMO, PADT, APPL, TAUT) and northern coast of English Channel up to Herstmonceux (PMTH, PRAE, EXMO, PBIL, POOL, CHIO, SANO, HERT) with one site (BRST) in the south. Two sites are in northern England (WEAR, LOFT), two in Scotland (LERI, BRAE) with one site in central Europe (ZIM2). All sites are equipped with GPS+GLONASS receivers. Note that sites CAMO, LERI and ZIM2 sites replace CAMB, LERW and ZIMM respectively, \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend were used by \cite{Penna2015}\DIFdelbegin \DIFdel{because the earlier siteswere replaced with newer ones, including adding GLONASS tracking. }\DIFdelend \DIFaddbegin \DIFadd{, to allow for use of GLONASS data recorded at the former set of sites.
}\DIFaddend 

\DIFaddbegin \DIFadd{Aside from the addition of GLONASS data, an important difference to the study of \mbox{%DIFAUXCMD
\cite{Penna2015} }\hspace{0pt}%DIFAUXCMD
is the shift in time period from 2007.0–2013.0 to 2010.0–2014.0. This shift provides sufficient GLONASS data following the upgrade of many receivers to track GLONASS from 2009 that followed the restoration of GLONASS constellation finished (24 SVs) in March 2010. Despite this covering a shorter time span, the length of continuous observations within each site (minimum availability of 0.95 through dataset) exceeds the recommended $\sim$1000 days of continuous observations (4 years with 0.7 availability) \mbox{%DIFAUXCMD
\citep{Penna2015}}\hspace{0pt}%DIFAUXCMD
. While the selected time period is fully covered by a complete set of reprocessed orbit and clock products.
}

\DIFaddend The chosen sites experience a range of $M_2$ up OTL amplitudes ranging from > 30 mm (ANLX, APPL, BRST, CAMO, PADT, PRAE), 15-30 mm (CARI, EXMO, LOFT, PBIL, SWAS, TAUT) and < 15 mm (BRAE, CHIO, LERI, POOL, SANO, WEAR, ZIM2).
...
Finally, OTL displacements modelled in GipsyX were added back using HARDISP \citep{IERS2010}. HARDISP uses spline interpolation of the tidal admittance of 11 major constituents to infer values of 342 tidal constituents and generate a time series of tidal displacements. This approach almost eliminates the effect of companion constituents \citep{Foreman1989} as they are modelled during the processing stage\DIFdelbegin \DIFdel{, even considering errors present in companion constituent displacement as tide model errors become neglectable for constituents this small}\DIFdelend \DIFaddbegin \DIFadd{; small errors in the modelled major OTL constituents will propagate into negligible errors in modeled companion tides}\DIFaddend . Thus, \DIFaddbegin \DIFadd{the }\DIFaddend analysed harmonic displacement parameters represent true displacement plus indiscernible companion constituent error, far below \DIFaddbegin \DIFadd{the }\DIFaddend measurement error. The findings in our paper are in the context of GipsyX software and solutions derived using other software may produce different results especially if the underlying model choices differ.

The harmonic analysis of the reconstructed OTL signal was performed using the Eterna software v.3.30 \citep{Wenzel1996}, resulting in amplitudes and local tidal potential phase lags negative which are suitable for solid Earth tide studies. OTL phase-lag, however, is defined with respect to the Greenwich meridian and phase lags \DIFdelbegin \DIFdel{positive.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The latter, however, is not important for the purposes of this publication as we ignore the long-period tides in the analysis but used them for internal Eterna testing; same goes for diurnal tides phase rotation as all sites’ latitudes are positivehere}\DIFdelend \DIFaddbegin \DIFadd{are positive}\DIFaddend . Transforming to Greenwich-relative lags was done according to \cite{Boy2003} and \cite{Bos2000}.
...
In Eq. 1 we assume body tide errors to be negligible, thus $Z_{obs}$ is simply an observed OTL and $Z_{th}$ is a theoretical OTL while $Z_{res}$, residual OTL, is their vector difference. $Z_{res}$ presented in this paper is, if not otherwise specified, relative to the theoretical OTL values computed using the FES2014b ocean tide atlas, a successor of FES2012 used in \cite{Bos2015}, and a Green’s function based on the STW105 Earth model additionally corrected for dissipation at the $M_2$ frequency which we call STW105d (FES2014b\_STW105d). \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend We utilize box-and-whisker plots to demonstrate the distribution of the estimates with the \DIFdelbegin \DIFdel{boxes defined by the interquartile }\DIFdelend \DIFaddbegin \DIFadd{box and whiskers defined as the inter-quartile }\DIFaddend range (IQR) \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and an additional }\pm \DIFadd{1.5\times \text{IQR}, respectively, }\DIFaddend with median as a horizontal line\DIFdelbegin \DIFdel{, and whiskers by additional }%DIFDELCMD < \pm %%%
\DIFdel{1.5\times \text{IQR}}\DIFdelend .
...
We now explore the sensitivity of our solutions to different products and analysis choices starting with elevation cutoff angle sensitivity, \DIFdelbegin \DIFdel{directly related to }\DIFdelend \DIFaddbegin \DIFadd{which particularly affects }\DIFaddend the amount of \DIFdelbegin \DIFdel{possible multipath presence}\DIFdelend \DIFaddbegin \DIFadd{multipath influence on the coordinate time series}\DIFaddend . We follow with intercomparison of various products \DIFdelbegin \DIFdel{solutions }\DIFdelend and assessing impact \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend integer ambiguity resolution (GPS only)\DIFdelbegin \DIFdel{to understand the possible associated inconsistencies}\DIFdelend . Finally, we test \DIFdelbegin \DIFdel{fidelity of Eterna software to various constituents and timeseries }\DIFdelend \DIFaddbegin \DIFadd{the stability of the constituent estimates to time series }\DIFaddend length. 
...
The same comparison done for GPS AR (7° and 20° cutoff, JPL native products) shows largely improved stability in comparison to all GPS only ambiguity free solutions (Figure 4, bottom). However, $K_2$ up and $K_1$ up show substantial differences between solutions: $K_2$ \DIFdelbegin \DIFdel{due to a lot }\DIFdelend \DIFaddbegin \DIFadd{shows as much }\DIFaddend smaller variance of $\|Z_{res}\|$ distribution in the 20° solution, possibly due to removal of multipath, and $K_1$ \DIFdelbegin \DIFdel{due to }\DIFdelend \DIFaddbegin \DIFadd{shows an }\DIFaddend increased dispersion and median of $\|Z_{res}\|$ at increased cutoff angle.
...
\DIFdelbegin \DIFdel{The importance of this assessment is directly related to the main principle of PPP technique: a priori knowledge of satellite-related parameters that are held fixed when estimating station coordinate time series. Given this approach, all satellite-related systematic errors will partially propagate into station-specific parameters \mbox{%DIFAUXCMD
...
GLONASS $K_2$ and $K_1$ estimates in the north have lowest variance in $\|Z_{res}\|$ and most stable \DIFdelbegin \DIFdel{towards changes in }\DIFdelend \DIFaddbegin \DIFadd{with different }\DIFaddend elevation cutoff angles and products. For the east component, CODE products \DIFdelbegin \DIFdel{GLONASS struggles }\DIFdelend \DIFaddbegin \DIFadd{with GLONASS struggle }\DIFaddend to compete with GPS+GLONASS in terms of $\|Z_{res}\|$ distribution variance ($K_1$) and elevation cutoff stability ($K_2$ and $K_1$). The ESA products GLONASS, however, performs better for $K_1$ east than the respective GPS+GLONASS in terms of $\|Z_{res}\|$ distribution consistency and median (Fig. S2). Elevation cutoff stability of ESA $K_2$ and $K_1$ in the east component is exactly as with CODE – best with GPS+GLONASS.

The up component of $K_2$ and $K_1$ is the most problematic, showing high $\|Z_{res}\|$ values with all constellation modes. GLONASS OTL values from both ESA and CODE solutions have \DIFaddbegin \DIFadd{the }\DIFaddend smallest $\|Z_{res}\|$ distribution variances and median values \DIFdelbegin \DIFdel{overperforming }\DIFdelend \DIFaddbegin \DIFadd{outperforming }\DIFaddend JPL GPS AR. Note that GPS+GLONASS $K_2$ up has \DIFaddbegin \DIFadd{a }\DIFaddend marginally smaller median $\|\Delta Z_{res}\|$ in the elevation cutoff test, possibly due to \DIFdelbegin \DIFdel{higher amount of SVs}\DIFdelend \DIFaddbegin \DIFadd{the larger number of total satellites}\DIFaddend , however, \DIFaddbegin \DIFadd{both $K_2$ and $K_1$ }\DIFaddend $\|Z_{res}\|$ \DIFdelbegin \DIFdel{values recovered are too high with major differences between CODE and ESA products’ values}\DIFdelend \DIFaddbegin \DIFadd{posses }\sim1\DIFadd{.5 mm bias}\DIFaddend .

While we cannot certainly select a single constellation configuration optimal for all components of $K_2$ and $K_1$, we can conclude that based on our analysis, GLONASS returns smaller $\|Z_{res}\|$ in the $K_2$ and $K_1$ north and up components while east component might show better results with GPS+GLONASS ($K_1$, CODE) but, due to \DIFdelbegin \DIFdel{higher closeness }\DIFdelend \DIFaddbegin \DIFadd{great agreement }\DIFaddend between products, \DIFdelbegin \DIFdel{GLONASS constellation is }\DIFdelend \DIFaddbegin \DIFadd{solutions using GLONASS constellation are }\DIFaddend still preferred. The only exception to the east component conclusions is GPS AR (see Sect. 5.7).
...
\caption{Comparison of residual constituents’ estimates from GPS (top) and GPS AR (bottom) JPL native solutions. Grey crosses are as per Figure 3. As seen, most of constituents’ $\|Z_{res}\|$ \DIFdelbeginFL \DIFdelFL{distributions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{distribution }\DIFaddendFL variances and medians \DIFdelbeginFL \DIFdelFL{got }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{are }\DIFaddendFL smaller while $S_2$ $\|Z_{res}\|$ bias \DIFdelbeginFL \DIFdelFL{got }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{is }\DIFaddendFL removed with \DIFdelbeginFL \DIFdelFL{enabling of }\DIFdelendFL AR \DIFaddbeginFL \DIFaddFL{solutions}\DIFaddendFL .}
\end{figure*}
\DIFaddbegin 
...
We assessed how $\|Z_{res}\|$ of each of 8 major constituents vary as a function of the time series length with kinematic estimation approach. The duration of the series varied by \DIFdelbegin \DIFdel{integral years , and}\DIFdelend \DIFaddbegin \DIFadd{integer years and, }\DIFaddend to enable a complete analysis, we expanded the candidate solutions to 2019.0 and processed additional data with operational products: JPL repro3.0, ESA operational, CODE MGEX (CODE operational lack GLONASS clock corrections). While the goal of reprocessing campaign \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend to preserve consistency with operational products \citep{Griffiths2019}, based on previous results, we assumed that changing satellite orbit and clock products may produce substantial differences in problematic solar-related constituents ($S_2$, $K_2$, $K_1$, $P_1$). Thus, we first performed a comparison of ESA repro2 solutions (2010.0-2014.0) with the ESA operational product (2014.0-2019.0) which confirmed the hypothesis (Figure 11). GLONASS $\|\Delta Z_{res}\|$ \DIFdelbegin \DIFdel{are still of minimal }\DIFdelend \DIFaddbegin \DIFadd{show the smallest }\DIFaddend variance for $K_1$ and $K_2$ compared with GPS and GPS+GLONASS but are significant, up specifically, which might be related to the changes in GLONASS products processing. Considering \DIFdelbegin \DIFdel{the }\DIFdelend $S_2$, the very same form of bias \DIFaddbegin \DIFadd{remains }\DIFaddend as previously seen with 2010.0-2014.0 dataset\DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{. This }\DIFaddend suggests symmetric deviation of repro2 and operational products solutions from \DIFaddbegin \DIFadd{the }\DIFaddend modelled value. The same explanation can be applied to the GPS-only $P_1$ up 0.5 mm $\|\Delta Z_{res}\|$ bias.

The results shown in Figure 12 are a composition of reprocessed products and operational products (years 5 to 9). We focus on $S_2$ up \DIFdelbegin \DIFdel{, to the evolution of inverted $\|Z_{res}\|$ bias }\DIFdelend and $K_1$ up, as \DIFaddbegin \DIFadd{the }\DIFaddend most problematic diurnal constituents. The results align with general conclusions of \cite{Yuan2013} suggesting \DIFdelbegin \DIFdel{weak relation }\DIFdelend \DIFaddbegin \DIFadd{a weak relationship }\DIFaddend between timeseries length and $\|Z_{res}\|$ for solar-related constituents\DIFdelbegin \DIFdel{which is likely to be caused by the changes in products concentrated over the four constituents as demonstrated on Fig 11. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend However, if constituents selected according to optimum constellation strategy, $\|Z_{res}\|$ appear (see Fig. S4) stable over time, which suggests that even if there are changes in the products, they are not having an impact with this methodology.
